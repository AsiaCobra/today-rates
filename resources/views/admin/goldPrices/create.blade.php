...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.goldPrice.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.gold-prices.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gold_type') ? 'is-invalid' : '' }}" name="gold_type" id="gold_type" required>
                    ...
                    @foreach(App\Models\GoldPrice::GOLD_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gold_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
