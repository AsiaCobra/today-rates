    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.currencies.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('currency_code') ? 'is-invalid' : '' }}" name="currency_code" id="currency_code" required>
                    ...
                    @foreach(App\Models\Currency::CURRENCY_CODE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('currency_code', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('buy_rate') ? 'is-invalid' : '' }}" type="number" name="buy_rate" id="buy_rate" value="{{ old('buy_rate', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sell_rate') ? 'is-invalid' : '' }}" type="number" name="sell_rate" id="sell_rate" value="{{ old('sell_rate', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('up_or_down') ? 'is-invalid' : '' }}" name="up_or_down" id="up_or_down">
                    ...
                    @foreach(App\Models\Currency::UP_OR_DOWN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('up_or_down', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
